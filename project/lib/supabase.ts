...
// In Jest/tests, prefer process.env to avoid importing native modules
const isTestEnv = !!process.env.JEST_WORKER_ID || process.env.NODE_ENV === 'test';
const supabaseUrl = (
  (isTestEnv ? process.env.SUPABASE_URL : Config.SUPABASE_URL) || ''
).trim();
const supabaseAnonKey = (
  (isTestEnv ? process.env.SUPABASE_ANON_KEY : Config.SUPABASE_ANON_KEY) || ''
).trim();

if (!supabaseUrl || !supabaseAnonKey) {
  // Throw early to prevent silent misconfiguration in production builds
  throw new Error(
    'Supabase environment variables are missing. Set SUPABASE_URL and SUPABASE_ANON_KEY in your .env file.'
  );
}
